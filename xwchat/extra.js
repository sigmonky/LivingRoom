...
function xcGetLocale(language) {
  if (!language) { language = 'en'; }; // always default to english if nothing given
...
function xcCreateForm(packet, name, onclick, form) {
  if (form) {
    var html = "<form id='" + name + "' name='" + name + "'>";
    html += "<table>";
  } else {
    var html = '';
  };
  ...
  JQ(packet.getDoc()).find('field').each(function() {
    ...
    if (type == 'fixed') {
      ...
      if (label)
        html += "<label for='"+ id + "' id='" + id + "_lbl'>" + label + ": </label>";
      html += "</td><td>";
    };
  ...
  if (form) {
    html += "</table>";
    html += "<br /><div align='center'><input type='button' name='submit_button' value='Submit' class='button' onClick='" + onclick + "();' /></div>";
    html += "</form>";
  };
  return html;
};
