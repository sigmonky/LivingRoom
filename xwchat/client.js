...
function xcSetMsg(msg, error) {
  (error) ? JQ('#msg').show().addClass('xcError').html(msg) : JQ('#msg').show().removeClass('xcError').html(msg);
  setTimeout('xcMsgClear()', XC.ERRORTIMEOUT);
};

/**
 * Clears the msg information from the screen
 */
function xcMsgClear() {
  JQ('#msg').hide().removeClass('xcError').html('');
};
...
function xcUserInfo(jid) {
  ...
  xcWinOpen('userdetails', 'jid=' + jid, 'USERDETAILS', 'USERDETAILS');
};
...
function xcUpdateRoster(iq) {
  ...
  if (check == 0) {
    JQ('div#Offline_contacts.xcGroupContact').append(xcBuildContactHtml(jid, contact));
  } else {
...
function xcMUCInvite(m) {
  ...
  if (JQ(m.getDoc()).find('decline').size() > 0) {
    ...
    xcWinOpen('muc.decline', 'from=' + from + '&reason=' + escape(reason), 'MUCDECLINE', 'MUCINVITE');
  } else {
    ...
    var reason = JQ(m.getDoc()).find('reason').text();
    xcWinOpen('muc.invite', 'from=' + from + '&room=' + room + '&reason=' + escape(reason), 'MUCINVITE', 'MUCINVITE');
  };
  return true;
};
...
function xcMUCInviteLaunch(name, nick) {
  ...
  xcWinOpen('muc', 'nick=' + nick, xcEnc(name), 'MUC');
};
...
function xcMsgHeadlines(m) {
  // FIXME: handle these types of messages and display on the screen
  oDbg.log("HeadLine Message:" + m.getDoc().xml);
  return true;
};
...
function xcMsg(m) {
  ...
  var fulljid = m.getFrom();
  var jid = xcJID(m.getFrom(), false);
  // checking if private group chat instance
  if (!(w = xcWinOpenGet(fulljid))) {
    if(!(contact = xcContactExists(jid))) {
      jid = fulljid; // private group chat
    };
    xcMsgUser(jid);
  };
  xcMsgProcess(w, m);
...
function xcPresenceSubscribe(p) {
  ...
  xcWinOpen('subscription', 'from=' + p.getFrom(), xcEnc(p.getFrom()) + 'SUBSCRIPTION', 'SUBSCRIPTION');
  return true;
};
...
function xcPresenceUnSubscribe(p) {
  if (typeof(p.getFrom()) == 'undefined' || p.getFrom() == '') { return true; };
  xcWinOpen('unsubscribe', 'from=' + p,getFrom(), xcEnc(p.getFrom()) + 'UNSUBSCRIBE', 'UNSUBSCRIBE');
  return true;
};
  ...
  if((contact = xcContactExists(jid))) {
    ...
    if (type == 'unavailable') {
      ...
      if (!(xcInOfflineGroup(jid))) {
        ...
        JQ('#' + xcEnc('Offline_contacts')).append(html);
      };
      JQ('.xcContact').each(function() {
        if (this.id == xcEnc(jid)) {
      ...
      if ((xcInOfflineGroup(jid))) {
        ...
        for (var x = 0; x < contact.group.length; x++) {
          ...
          var html = xcBuildContactHtml(jid, contact); // rebuild the html for the contact
          JQ('#' + xcEnc(gn + '_contacts')).append(html);
        };
      };
      JQ('.xcContact').each(function() {
        if (this.id == xcEnc(jid)) {
...
function xcError(e) {
  ...
  if (code == 401) {
    xcSetMsg(xcT('Wrong combination Username / Password'), true);
  };
  return true;
};
...
function xcConnected() {
  try {
    xcSetUserPresence('available')
    con.send(new JSJaCPresence()); // sending initial presence packet
    ...
    setTimeout(function() {
      ...
      xcGetLocale(XC.locale); // getting the saved locale from the cookie and setting it
      ...
      if (JQ(document).getUrlParam('buddy')) {
        xcMsgUser(JQ(document).getUrlParam('buddy'));
      };
    }, XC.CONFIG_WAIT);
  } catch (e) {
    xcSetMsg(e.message, true);
...
function xcLogin() {
  try {
    ...
    if(JQ(document).getUrlParam('domain')) { oArgs.domain = JQ(document).getUrlParam('domain'); };
...
function xcRosterDisplay() {
  ...
  for (var x = 0; x < contactlist.length; x++) {
    ...
    if (JQ(check).size() == 0) {
      ...
      JQ('#' + xcEnc('Offline_contacts')).append(html).parent().show();
...
function xcViewUserLog(jid) {
  ...
  xcWinOpen('archive', 'jid=' + escape(jid), xcEnc(jid), 'ARCHIVE');
};

/**
 * Opens one on one user chat window
 * @param {String} jid bare jabberid of the user we wish to chat with
 */
function xcMsgUser(jid) {
  if(!jid) { return false; };
  if ((w = xcWinOpenGet(jid))) {
    w.focus();
    return false;
  };
  xcWinOpen('message', 'jid=' + escape(jid), xcEnc(jid), 'MSG');
};
...
function xcWinUpdateUserOpen(jid) {
  ...
  xcWinOpen('updateuser', 'jid=' + jid, 'UPDATEUSER', 'UPDATEUSER');
};
...
function xcWinUpdatePresenceOpen(jid) {
  ...
  xcWinOpen('presence.other', 'jid=' + jid, 'PRESENCEOTHER', 'PRESENCEOTHER');
};
...
function xcGetVCardNickName() {
  try {
    ...
    con.send(iq, window.xcGetVCardNickNameVrfy);
  } catch (e) {
    xcSetMsg(e.message, true);
  };
};
...
/**
 * set the global variable XC.nickname so we can have access to it when needed
 * @param {JSJaCIQ} iq IQ stanza with relevant VCard information
 */
function xcGetVCardNickNameVrfy(iq) {
